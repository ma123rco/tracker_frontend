...
const backToDefaultRoute = async() => {
    try {
        if (['gerente', 'admin'].includes(useUserData.userData?.user?.role)) await router.push({ name: "home", force: true });
        else await router.push({ name: "home", force: true });
    } catch (e) {
...
<template>
    <div class="flex min-h-screen items-center justify-center bg-cover bg-center bg-no-repeat"
        ...
        <div class="mx-2 max-w-md rounded-lg bg-white bg-opacity-90 p-4 text-center shadow-lg text-surface-900 dark:bg-surface-900 dark:text-surface-100">
            ...
            <Button fluid @click="backToDefaultRoute">Regresar</Button>
        </div>
    </div>
</template>
